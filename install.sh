...
_ensure_prereqs() {
  ...
  _ensure_cmd_or_install jq jq
  _ensure_cmd_or_install yq yq
}
...
#   - In non-interactive mode, local changes are stashed automatically.
#   - Uses rebase and autostash for a cleaner automation path.
# ----------------------------
_update_repo() {
  local action=""
  local commit_msg=""
...
#   - Also applies configs/executable.list when present.
# ----------------------------
_ensure_executables() {
  local root="${ROOT_DIR}"
...
# ----------------------------
# Render canonical state env content
# Notes:
#   - Keeps config/examples/state.env.example and state/configs/.env aligned.
#   - Example file contains safe placeholders and defaults only.
#   - Local .env contains the effective installer values for this host.
# ----------------------------
_render_state_env_content() {
  local mode="${1:?Missing mode}"
  local repo_dir_value=""
  ...
  case "${mode}" in
    ...
    local)
      ...
      ;;
    *)
      _error "Unsupported state env render mode: ${mode}"
      return 1
      ;;
  esac
  ...
  cat <<EOF
# ################################################################
# File: config/examples/state.env.example
# Created: 2026/03/07
# Updated: 2026/03/07
# Description:
#   Canonical local environment settings for homelab bootstrap and runtime.
# Notes:
#   - Generated by install.sh to keep the example and active local env aligned.
...
ENABLE_OPENBAO_BOOTSTRAP=0
OPENBAO_ADDR=
OPENBAO_CACERT=state/secrets/openbao/tls/ca/ca.pem
OPENBAO_SKIP_VERIFY=false
OPENBAO_INIT_SHARES=5
OPENBAO_INIT_THRESHOLD=3
OPENBAO_TRANSIT_MOUNT=transit
OPENBAO_TRANSIT_KEY=autounseal
OPENBAO_TRANSIT_POLICY=autounseal
OPENBAO_TRANSIT_TOKEN_PERIOD=720h
EOF
}
